...
#+begin_src emacs-lisp (unless (version< emacs-version "27") :tangle ~/.emacs.d/early-init.el)
...
#+begin_src emacs-lisp (when (eq system-type 'windows-nt) :tangle no)
...
#+begin_src emacs-lisp (unless (version< emacs-version "28") :tangle no)
