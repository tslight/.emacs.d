...
A fairly standard literate Emacs configuration using [[https://orgmode.org/][Org mode]].

Probably won't work well on Emacs < 27.1 ...

Quickest way to get setup is to back up your current config, then visit this
file and run ~org-babel-tangle~ with ~C-c C-v t~.

That will tangle this file and overwrite whatever you have in your
~$HOME/.emacs.d/early-init.el~ and ~$HOME/.emacs.d/init.el~, so make sure
you've backed up anything you don't want to lose!
